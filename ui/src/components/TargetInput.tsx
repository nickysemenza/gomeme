...
import { ImageInput, TextInput } from "../proto/meme_pb";
...
const TargetInput: React.FC<Props> = ({ target, index, onUpdate }) => {
  const handleInputChange = (value: string) => {
    ...
    if (isImage) {
      const imageInput = target.image || new ImageInput();
      imageInput.setUrl(value);
      onUpdate({ image: imageInput, kind: "image" });
    } else {
      const textInput = target.text || new TextInput();
      if (textInput.getColor() === "") textInput.setColor("#9859e0");
      textInput.setText(value);
      onUpdate({ text: textInput, kind: "text" });
    }
  };
      ...
      <div className="space-y-4">
        <div>
          ...
          <input
            ...
            placeholder={target.kind === 'image' ? 'https://... or data:image/...' : 'Enter text here...'}
            value={target.text?.getText() || target.image?.getUrl() || ""}
        ...
        <div>
          ...
          {target.image && (
            <ImagePreview image={target.image} />
          )}
        </div>
      </div>
    </div>
    ...
    <div className="flex items-center space-x-4">
      <div className="w-12 h-12 rounded-lg border-2 border-gray-200 overflow-hidden">
        <div 
          ...
          style={{ backgroundColor: text.getColor() }}
        />
      </div>
